...
class DataSource {
  ...
  async results () {
    let results = await this.query(`select * from NLP_BENCHMARK
    where run in (
      select * from (
        (select run from (select run, RIGHT(run, 19) as rundate, platform from NLP_BENCHMARK group by run) as runs WHERE platform = 'alexa' ORDER BY rundate desc LIMIT 1)
        UNION 
        (select run from (select run, RIGHT(run, 19) as rundate, platform from NLP_BENCHMARK group by run) as runs WHERE platform = 'google' ORDER BY rundate desc LIMIT 1)
        UNION 
        (select run from (select run, RIGHT(run, 19) as rundate, platform from NLP_BENCHMARK group by run) as runs WHERE platform = 'openai' ORDER BY rundate desc LIMIT 1)
      ) as n
    ) and IGNORE_QUESTION <> 'TRUE'
    order by UTTERANCE`)
